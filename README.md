# BitLocker Startup PIN — Intune Win32 App
...
| **Version** | 3.1.0 |
| **Tests** | 89 passing (`Test-BitLockerPinApp.ps1`) |
...
```
NONCOMPLIANT: no startup PIN protector - user has not set a PIN | device=WKH-00051
...
`WKH-0051` into `1110051`. Such a PIN is guessable by anyone who knows the naming scheme, so it only
...
89 cases: PIN derivation, the FVE plan, junction and ownership handling, the `ServiceUI.exe`
